...
pub export fn setcontext(ctx: *const c.ucontext_t) c_int {
    ...
    // 使用 RSI 作为 gregs 基址，因为 RDI 需要先恢复
    asm volatile (
        \\  // 将 gregs 地址移到 RSI
        \\  movq %[gregs], %%rsi
        \\
        \\  // 恢复所有寄存器（按照 musl 的标准顺序）
        \\  // 注意：必须最后恢复 RSP 和 RIP，因为恢复 RSP 后会切换栈
        ...
        \\  movq 72(%%rsi), %%rax        // REG_RSI（临时，因为 RSI 还在用）
        \\  movq 80(%%rsi), %%rbp        // REG_RBP
        \\  movq 88(%%rsi), %%rbx        // REG_RBX
        \\  movq 96(%%rsi), %%rdx        // REG_RDX
        \\  movq 104(%%rsi), %%rcx       // REG_RAX（临时存储到 RCX）
        \\
        \\  // 恢复 RSI（在恢复 RSP 之前，因为还需要从 RSI 读取）
        \\  movq %%rax, %%rsi            // REG_RSI
        \\
        \\  // 恢复 EFLAGS
        \\  pushq 136(%%rsi)             // REG_EFL
        \\  popfq
        \\
        \\  // 恢复 RCX（之前临时存储的 RAX）
        \\  movq %%rcx, %%rax            // REG_RAX
        \\  movq 112(%%rsi), %%rcx       // REG_RCX
        \\
        \\  // 最后恢复栈指针和跳转（musl 方式：直接跳转，不通过栈）
        \\  movq 120(%%rsi), %%rsp       // REG_RSP（切换栈！）
        \\  movq 128(%%rsi), %%rax       // REG_RIP（临时存储到 RAX）
        \\  jmpq *%%rax                  // 跳转到 RIP
        :
        : [gregs] "r" (gregs),
        : "memory", "cc"
...
pub export fn makecontext(ctx: *c.ucontext_t, func: ?*const fn () callconv(.C) void, argc: c_int, ...) void {
    ...
    var sp: usize = stack_bottom + ctx.uc_stack.ss_size;
    sp = sp & ~(alignment - 1);
    ...
    // 计算 MakecontextInfo 的位置（在栈顶下方，预留足够的空间）
    // musl 不预留额外的栈帧空间，因为 makecontext 的函数会在新栈上运行
    const info_ptr = sp - aligned_info_size;
