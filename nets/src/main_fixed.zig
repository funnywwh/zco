...
pub fn main() !void {
    std.log.info("Starting ZCO HTTP Server (Fixed Version)...", .{});
...
// 协程池限制 - 限制同时存在的协程数量
const MAX_COROUTINES = 100;
var activeCoroutines: std.atomic.Value(u32) = std.atomic.Value(u32).init(0);
    ...
    _ = try schedule.go(struct {
        fn run(s: *zco.Schedule) !void {
            ...
            while (true) {
                ...
                // 检查协程数限制
                const currentCoroutines = activeCoroutines.load(.monotonic);
                if (currentCoroutines >= MAX_COROUTINES) {
                    std.log.warn("Max coroutines reached, dropping connection", .{});
                    client.close();
                    client.deinit();
                    continue;
                }
                ...
                _ = activeCoroutines.fetchAdd(1, .monotonic);
                std.log.debug("accept a client, connections: {}, coroutines: {}", .{ currentConnections + 1, currentCoroutines + 1 });

                _ = try _s.go(struct {
                    fn run(_client: *nets.Tcp) !void {
                        defer {
                            _ = connectionCount.fetchSub(1, .monotonic);
                            _ = activeCoroutines.fetchSub(1, .monotonic);
