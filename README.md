...
### 🚀 ucontext 实现优化
- **架构优化**: 移除 ucontext 实现中的信号屏蔽处理，由调用者统一管理信号
- **性能提升**: 减少不必要的系统调用，提高上下文切换效率
- **职责分离**: 信号处理逻辑集中在调度器层，代码更清晰
- **向后兼容**: API 完全兼容，无需修改现有代码

### 📚 文档完善
- **实现文档**: 完整的 `docs/UCONTEXT_IMPLEMENTATION.md` 实现文档
- **技术细节**: 详细说明寄存器保存/恢复机制和栈布局

## 🆕 v0.5.0 更新内容

### 🔧 自实现 ucontext API
- **独立实现**: 使用内联汇编重新实现了 `getcontext`、`makecontext`、`swapcontext` 函数
- **完整支持**: 包括浮点数寄存器（XMM0-XMM15）的完整保存和恢复
- **零依赖**: 不再依赖系统 `ucontext` 库，完全自主可控
- **性能优化**: 使用 `fxsave`/`fxrstor` 指令高效保存/恢复浮点状态
- **详细文档**: 参见 `docs/UCONTEXT_IMPLEMENTATION.md`
...
- [x] 时间片抢占调度机制
- [x] 协程状态管理和上下文切换
- [x] 自实现 ucontext API（v0.5.0，包括浮点寄存器支持）
- [x] ucontext 实现优化（v0.6.0，移除信号屏蔽处理，提升性能）
- [x] 信号屏蔽保护共享数据
- [x] 性能统计和监控
- [x] 批量协程处理优化
- [x] 网络服务器集成
